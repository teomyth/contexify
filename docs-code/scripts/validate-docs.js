...
const EN_DOCS_DIR = path.resolve(__dirname, '../../docs-site/i18n/en/docusaurus-plugin-content-docs/current');
...
function runTestFile(filePath) {
  try {
    execSync(`npx ts-node-esm ${filePath}`, { stdio: 'inherit' });
    return true;
  } catch (error) {
...
function main() {
  ...
  for (const docFile of docFiles) {
    ...
    for (let i = 0; i < codeBlocks.length; i++) {
      ...
      console.log(`Testing file ${i + 1}/${codeBlocks.length}: ${testFilePath}`);
