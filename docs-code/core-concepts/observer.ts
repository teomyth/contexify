...
class ServiceRegistry implements ContextObserver {
  ...
  filter = (binding: Readonly<Binding<unknown>>) =>
    binding.tagMap.service != null;
...
async function run() {
  ...
  context
    .bind('services.OrderService')
    .to({ name: 'OrderService' })
    .tag('service');

  context.bind('config.api').to({ port: 3000, host: 'localhost' });
