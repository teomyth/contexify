...
class InterceptClassDecoratorFactory extends ClassDecoratorFactory<
  InterceptorOrKey[]
> {
  protected mergeWithOwn(ownMetadata: InterceptorOrKey[], _target: object) {
...
> {
  protected mergeWithOwn(
    ...
    _target: object,
    methodName: string,
    _methodDescriptor: TypedPropertyDescriptor<unknown>
  ) {
    ownMetadata = ownMetadata || {};
...
export function intercept(...interceptorOrKeys: InterceptorOrKey[]) {
  return function interceptDecoratorForClassOrMethod(
    ...
    // Using any for TypedPropertyDescriptor
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
